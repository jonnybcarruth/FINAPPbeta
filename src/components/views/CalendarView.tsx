...
export default function CalendarView() {
  ...
  const handleDelete = (id: string) => {
    ...
    setOneTimeTransactions(oneTimeTransactions.filter((t) => t.id !== id));
    setShowDay(false);
    setTimeout(saveAndRefresh, 0);
  };
    ...
    <div className="space-y-6">
      <section className="bg-white dark:bg-gray-800 py-6 px-0 rounded-2xl shadow-sm">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 px-6">
          ...
          <button onClick={() => { setEditTx(null); setDefaultDate(format(currentCalendarDate, 'yyyy-MM-dd')); setShowOneTime(true); }} className="px-4 py-2 bg-dindin-green text-white rounded-lg hover:bg-dindin-green-dark font-semibold">
            Add One-Time Event
          </button>
        </div>
          ...
          <div className="flex space-x-3">
            {(['small', 'medium', 'large'] as const).map((sz) => (
              <button key={sz} onClick={() => { setSettings({ ...settings, calendarSize: sz }); }}
                className={`px-4 py-1 text-sm rounded-full ${settings.calendarSize === sz ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800 hover:bg-gray-300'}`}>
