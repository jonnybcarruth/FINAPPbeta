...
export default function SpendingPlanView() {
  ...
  const handleSlider = (v: number) => {
    setSettings({ ...settings, savedAmount: v });
    setTimeout(saveAndRefresh, 0);
  };

  const handleCategoryToggle = (name: string, checked: boolean) => {
    setActiveSpendingCategories(activeSpendingCategories.map((c) => c.name === name ? { ...c, enabled: checked } : c));
    setTimeout(saveAndRefresh, 0);
  };
  ...
  const handleDeletePlan = () => {
    ...
    setOneTimeTransactions(oneTimeTransactions.filter((t) => !t.name.includes('(Planned)')));
    setTimeout(saveAndRefresh, 0);
  };
    ...
    <div className="space-y-6">
      ...
      <section className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={handleDeletePlan} className="px-3 py-1 text-sm bg-red-100 text-red-700 rounded-lg hover:bg-red-200">Delete Plan</button>
        </div>
        {remainingBudget <= 0 ? (
